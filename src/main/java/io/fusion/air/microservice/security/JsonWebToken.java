...
public final class JsonWebToken {
	...
	private boolean useSecretKey = true;
	...
	public JsonWebToken init(int _tokenType) {
		...
		useSecretKey = (tokenType == SECRET_KEY);
		if(useSecretKey) {
			macAlgorithm = Jwts.SIG.HS512;
		} else {
			signatureAlgorithm = Jwts.SIG.RS256;
		}

		System.out.println("Token Type = "+tokenType+" Algorithm = "+(useSecretKey ? macAlgorithm : signatureAlgorithm));
	...
	public String getAlgorithmId() {
		return useSecretKey ? macAlgorithm.getId() : signatureAlgorithm.getId();
	}
    ...
    public String generateToken(String _userId, String _issuer, long _expiryTime, Map<String, Object> _claims) {
		...
		if(useSecretKey) {
			builder.signWith((SecretKey) signingKey, macAlgorithm);
		} else {
			builder.signWith((java.security.PrivateKey) signingKey, signatureAlgorithm);
		}
    ...
    public void tokenStats(String token, boolean showClaims, boolean showPayload) {
		...
		System.out.println("Signature  : " + jws.getDigest());
		if(showClaims) {
			Claims claims = getAllClaims(token);
			int x = 1;
